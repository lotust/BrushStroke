...
Looking to mix up a backend with express/sequelize and a frontend with react/redux? That's `inkstone`!
...
```
git remote add inkstone https://github.com/FullstackAcademy/inkstone.git
git fetch inkstone
git merge inkstone/master
```

Why did we do that? Because every once in a while, `inkstone` may be updated with additional features or bug fixes, and you can easily get those changes from now on by entering:
...
* Create two postgres databases: `inkstone` and `inkstone-test` (you can substitute these with the name of your own application - just be sure to go through and change the `package.json` and `.travis.yml` to refer to the new name)
  * By default, running `npm test` will use `inkstone-test`, while regular development uses `inkstone`
...
CI is not about testing per se – it's about _continuously integrating_ your changes into the live application, instead of periodically _releasing_ new versions. CI tools can not only test your code, but then automatically deploy your app. inkstone comes with a `.travis.yml` configuration almost ready for deployment; follow these steps to complete the job.
...
Some developers may prefer to control deployment rather than rely on automation. Your local copy of the application can be pushed up to Heroku at will, using inkstone's handy deployment script:
